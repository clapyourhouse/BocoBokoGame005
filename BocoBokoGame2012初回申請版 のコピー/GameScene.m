...
- (id)init
{
    if ( self = [super init] ) {
        ...
        // BGM再生
        [[SimpleAudioEngine sharedEngine] playBackgroundMusic:@"bgm_GameScene.mp3"];
        ...
        theboss = [CCSprite spriteWithFile:@"theboss_mains.png"];
        theboss.position = CGPointMake(dispSize.width / 2.4, dispSize.height / 1.8);
        [self addChild:theboss z:200];
...
-(void)updatescore
{
    NSString *scoreString = [NSString stringWithFormat:@"%08d", score_theboss];
...
- (BOOL)ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if ( CGRectContainsPoint(rect, location) ) {
                 ...
                 count == 57 || count == 60 || count == 63 || count == 66 || count == 69 || count == 72)
        {
            ...
            if (count == 3) {
                coinSprite = [CCSprite spriteWithFile:@"suica-4s.png"];
            }else if (count == 18) {
                coinSprite = [CCSprite spriteWithFile:@"hbook-4s-1.png"];
            }
        ...
        if ( bossNumber == 1 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"theboss_lefts.png"];
        }
        else if( bossNumber == 2 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"theboss_rights.png"];
        }
        else if( bossNumber == 3 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"theboss_tops.png"];
        }
        else if( bossNumber == 4 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"theboss_bottoms.png"];
        }
        // theboss_falldownの画像はサイズが大きすぎるので入りきらないため入れない
        else if( bossNumber == 5 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"theboss_falldowns.png"];
        }
        
        
        [theboss setTexture:texture];
...
- (void)nextScene
{
    ...
    CCScene *newScene = [GameSceneSecond node];
