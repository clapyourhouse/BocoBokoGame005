...
@implementation GameSceneSecond


@synthesize scoreLabel;
@synthesize score_director;

+ (id)scene
{
    CCScene *scene = [CCScene node];
    CCLayer *gameScene = [GameSceneSecond node];
...
- (id)init
{
    if ( self = [super init] ) {
        ...
        // BGM再生
        [[SimpleAudioEngine sharedEngine] playBackgroundMusic:@"bgm2_GameScene.mp3"];
        
        // iPhoneの画面サイズを取得する
        CGSize dispSize = [[CCDirector sharedDirector] winSize];
        
        // CCSpriteを使って背景画像を作成し、位置・サイズを調整して貼り付け
        CCSprite *back = [CCSprite spriteWithFile:@"background_office.png"];
        ...
        director = [CCSprite spriteWithFile:@"director_mains.png"];
        director.position = CGPointMake(dispSize.width / 2.4, dispSize.height / 1.4);
        [self addChild:director z:150];
...
-(void)updateScore
{
    NSString *scoreString = [NSString stringWithFormat:@"%08d", score_director];
...
- (BOOL)ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if ( CGRectContainsPoint(rect, location) ) {
                 ...
                 count == 56 || count == 59 || count == 62 || count == 65 || count == 68 || count == 71)
        {
            ...
            if (count == 5) {
                coinSprite = [CCSprite spriteWithFile:@"tabako-4s.png"];
            }else if (count == 8) {
                coinSprite = [CCSprite spriteWithFile:@"zip-4s-1.png"];
            }else if (count == 11) {
                coinSprite = [CCSprite spriteWithFile:@"daiya-white-4s-1.png"];
            }else if (count == 14) {
                coinSprite = [CCSprite spriteWithFile:@"daiya-blue-4s-1.png"];
            }else if (count == 17) {
                coinSprite = [CCSprite spriteWithFile:@"daiya-pink-4s-1.png"];
            }
        ...
        if ( bossNumber == 1 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"director_lefts.png"];
        }
        else if( bossNumber == 2 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"director_rights.png"];
        }
        else if( bossNumber == 3 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"director_tops.png"];
        }
        else if( bossNumber == 4 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"director_bottoms.png"];
        }
        else if( bossNumber == 5 )
        {
            texture = [[CCTextureCache sharedTextureCache] addImage:@"director_falldowns.png"];
        }
...
- (void)nextScene
{
    ...
    [ud setInteger:score_director forKey:@"score_director"];
    [ud synchronize];
    
    CCScene *newScene = [GameSceneLast node];
    [[CCDirector sharedDirector] replaceScene:[CCTransitionFadeBL transitionWithDuration:2 scene:newScene]];
}
